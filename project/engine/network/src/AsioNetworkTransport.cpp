...
AsioNetworkTransport::AsioNetworkTransport(const std::string &bindAddress,
                                           uint16_t bindPort)
    : _bindAddress(bindAddress), _bindPort(bindPort), _socket(_io),
      _workGuard(asio::make_work_guard(_io)) {
  try {
    udp::endpoint ep(asio::ip::make_address(bindAddress), bindPort);
    _socket.open(ep.protocol());
    _socket.bind(ep);
    Logger::info("[AsioTransport] Bound to " + bindAddress + ":" +
                 std::to_string(bindPort));
  } catch (const std::exception &e) {
    Logger::error(std::string("[AsioTransport] Failed to bind socket: ") +
                  e.what());
    throw;
  }
}

AsioNetworkTransport::~AsioNetworkTransport() { stop(); }
...
void AsioNetworkTransport::start() {
  ...
  _thread = std::thread([this]() {
    try {
      ...
      Logger::error(std::string("[AsioTransport] IO thread crashed: ") +
                    e.what());
    }
  });
}
...
void AsioNetworkTransport::stop() {
  ...
  try {
    ...
    Logger::error(std::string("[AsioTransport] Exception in stop(): ") +
                  e.what());
  }
}
...
void AsioNetworkTransport::sendTo(const endpoint_t &to,
  ...
  _socket.async_receive_from(
      ...
      [this](std::error_code ec, std::size_t bytes_recvd) {
        ...
        if (!ec && bytes_recvd > 0) {
          ...
          if (_cb) {
            std::vector<uint8_t> data(_recvBuf.data(),
                                      _recvBuf.data() + bytes_recvd);
            try {
              _cb(data, e);
            } catch (const std::exception &ex) {
              Logger::warn(
                  std::string("[AsioTransport] recv callback exception: ") +
                  ex.what());
            }
          }
