...
namespace Network {
    ...
    ReliableLayerAdapter::ReliableLayerAdapter(std::shared_ptr<INetworkTransport> transport,
                                               std::shared_ptr<IMessageSerializer> serializer,
                                               size_t mtu)
        : _transport(std::move(transport)), _serializer(std::move(serializer)), _mtu(mtu)
    {
        if (!_transport) {
        ...
        _transport->setRecvCallback([this](const std::vector<uint8_t>& data, const endpoint_t& from) {
    ...
    uint32_t ReliableLayerAdapter::sendReliable(const endpoint_t& to, const Network::Packet& pkt)
    {
        ...
        if (!peerPtr) {
            Logger::warn("[Adapter] sendReliable: failed to get/create peer for " + key);
            return 0;
        }
    ...
    void ReliableLayerAdapter::sendUnreliable(const endpoint_t& to, const Network::Packet& pkt)
    {
        std::vector<uint8_t> raw;
        try {
            raw = _serializer->serialize(pkt);
        } catch (const std::exception& e) {
            Logger::warn(std::string("[Adapter] serialize unreliable failed: ") + e.what());
            return;
        }
        if (_transport)
    ...
    void ReliableLayerAdapter::onTransportRecv(const std::vector<uint8_t>& data, const endpoint_t& from)
    {
        if (data.empty())
            return;
        ...
        try {
            ...
            if (msgType == static_cast<uint8_t>(Network::RELIABLE_FRAGMENT)) {
                ...
                if (fragLen != fragPayloadLen) {
                    Logger::warn("[Adapter] fragment length mismatch: header=" + std::to_string(fragLen) +
                                 " actual=" + std::to_string(fragPayloadLen));
                }

                auto opt = peerPtr->channel.onIncomingFragmentPayload(fragPayloadPtr, fragPayloadLen);
                if (opt) {
                    ...
                    if (_transport) {
                        ...
                        Logger::debug("[Adapter] Sent ACK for seq=" + std::to_string(parentSeq));
                    }
                } else {
            ...
            if (msgType == static_cast<uint8_t>(Network::ACK)) {
                if (data.size() >= 1 + 4) {
                    ...
                    Logger::debug("[Adapter] Received ACK for seq=" + std::to_string(ackSeq));
            ...
            Logger::warn(std::string("[Adapter] Deserialize/processing failed: ") + e.what());
            return;
        }
    }
