...
System::ServerNetworkSystem::ServerNetworkSystem(unsigned short port) : _port(port) {}

void System::ServerNetworkSystem::init(Ecs::Registry&)
{
    _transport = std::make_shared<Network::AsioNetworkTransport>("0.0.0.0", _port);
    _serializer = std::make_shared<Network::DefaultMessageSerializer>();
    _adapter = std::make_unique<Network::ReliableLayerAdapter>(_transport, _serializer, 1200);
    ...
    _handlers[Network::NEW_CLIENT] = [this](const Network::Packet& pkt, const Network::endpoint_t& from) {
    ...
    _adapter->setAppPacketCallback([this](const Network::Packet& pkt, const Network::endpoint_t& from) {
        onAppPacket(pkt, from);
    });

    _transport->start();
    Logger::info("[Server] Network system started on port " + std::to_string(_port));
}
...
void System::ServerNetworkSystem::onAppPacket(const Network::Packet& pkt, const Network::endpoint_t& from)
{
    auto it = _handlers.find(pkt.header.type);
    if (it != _handlers.end())
        it->second(pkt, from);
    else
        Logger::warn("[Server] Unknown packet type: " + std::to_string(pkt.header.type));
}

void System::ServerNetworkSystem::handleNewClient(const Network::Packet&, const Network::endpoint_t& from)
{
    std::string key = from.address + ":" + std::to_string(from.port);
...
void System::ServerNetworkSystem::handleDisconnect(const Network::Packet& pkt, const Network::endpoint_t& from)
{
    ...
    Logger::info("[Server] Client " + key + " disconnecting (room=" + std::to_string(roomId) + ", id=" + std::to_string(clientId) + ")");

    _rooms->leave(roomId, clientId);
}

extern "C" std::shared_ptr<System::ISystem> createServerNetworkSystem(std::any params)
{
    ...
    try {
        ...
        Logger::error(std::string("[Factory]: Failed to create ServerNetworkSystem: ") + e.what());
    }
    return nullptr;
}
