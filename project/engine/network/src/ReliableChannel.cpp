...
Network::ReliableChannel::ReliableChannel(size_t mtu, double rto, unsigned maxRetries, size_t window)
    : _mtu(mtu), _rto(rto), _maxRetries(maxRetries), _windowSize(window), _nextSeq(1)
{}

uint32_t Network::ReliableChannel::queueReliable(const std::vector<uint8_t>& payload)
{
...
std::vector<Network::ReliableChannel::outgoing_datagram_t> Network::ReliableChannel::collectOutgoing()
{
    std::lock_guard<std::mutex> lk(_mtx);
...
void Network::ReliableChannel::tick(double dt)
{
    ...
    for (auto it = _pendings.begin(); it != _pendings.end(); it++) {
        ...
        auto elapsed = duration_cast<milliseconds>(now - p.lastSend).count() / 1000.0;
...
std::optional<std::pair<uint32_t, std::vector<uint8_t>>> Network::ReliableChannel::onIncomingFragmentPayload(const uint8_t* data, size_t size)
{
    if (size < 8)
        return std::nullopt;
    ...
    entry.parts[fragId] = std::vector<uint8_t>(payloadPtr, payloadPtr + payloadLen);
...
std::vector<std::vector<uint8_t>> Network::ReliableChannel::buildFragments(uint32_t parentSeq, const std::vector<uint8_t>& payload)
{
    size_t headerOverhead = 8;
    size_t maxDataPerFragment = (_mtu > headerOverhead) ? (_mtu - headerOverhead) : 32;
    size_t total = payload.size();
    uint16_t fragmentCount = static_cast<uint16_t>((total + maxDataPerFragment - 1) / maxDataPerFragment);
    ...
    for (uint16_t fid = 0; fid < fragmentCount; fid++) {
        ...
        auto frag = buildFragmentPayload(parentSeq, fid, fragmentCount, payload.data() + offset, chunk);
...
std::vector<uint8_t> Network::ReliableChannel::buildFragmentPayload(uint32_t parentSeq, uint16_t fragId, uint16_t fragCount, const uint8_t* data, size_t len)
{
