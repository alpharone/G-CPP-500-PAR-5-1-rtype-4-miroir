...
TEST_CASE("Registry::spawnEntity produces increasing ids and reuses freed ids",
          "[Registry][spawn][kill]") {
  Ecs::Registry reg;
...
TEST_CASE("Component registration, emplaceComponent and removeComponent",
          "[Registry][components]") {
  Ecs::Registry reg;
...
TEST_CASE("killEntity triggers erase callbacks that remove components",
          "[Registry][kill][callbacks]") {
  Ecs::Registry reg;
...
TEST_CASE("addSystem and runSystems: system can iterate / modify components",
          "[Registry][systems]") {
  Ecs::Registry reg;
  ...
  reg.addSystem<Position>(
      [](Ecs::Registry &r, Ecs::SparseArray<Position> &positions) {
...
TEST_CASE("getComponents const overload throws when component not registered",
          "[Registry][const][get]") {
  Ecs::Registry reg;
  auto e0 = reg.spawnEntity();
  ...
  REQUIRE_THROWS_AS((void)creg.getComponents<Unregistered>(),
                    std::out_of_range);
}