...
namespace std {
    template <> struct hash<Network::endpoint_t> {
        std::size_t operator()(Network::endpoint_t const& e) const noexcept {
            return std::hash<std::string>()(e.address) ^ (std::hash<uint16_t>()(e.port) << 1);
        }
    };
}
...
namespace Network {
    ...
    public:
        ...
        uint32_t createRoom();
        std::optional<std::pair<uint32_t, uint32_t>> joinAuto(const endpoint_t& endpoint);
        ...
        void onPacket(uint32_t roomId, const endpoint_t& from, const Network::Packet& p);
        void tick(double dt);
