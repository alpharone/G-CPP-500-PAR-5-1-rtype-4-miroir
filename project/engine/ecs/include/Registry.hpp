...
public:
  ...
  template <typename Component> SparseArray<Component> &registerComponent() {
    std::type_index key(typeid(Component));

    auto [it, inserted] =
        _components.try_emplace(key, std::make_any<SparseArray<Component>>());
    if (inserted) {
  ...
  auto emplaceComponent(Entity const &e, Params &&...params) {
    return getComponents<Component>().emplace_at(
        static_cast<size_t>(e), std::forward<Params>(params)...);
  }

  template <typename Component> void removeComponent(Entity const &e) {
