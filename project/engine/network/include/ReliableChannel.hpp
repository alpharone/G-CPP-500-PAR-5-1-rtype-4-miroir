...
namespace Network {
    ...
    class ReliableChannel {
        public:
            ...
            ReliableChannel(size_t mtu = 1200, double rto = 0.25, unsigned maxRetries = 5, size_t window = 32);
            ...
            std::optional<std::pair<uint32_t, std::vector<uint8_t>>> onIncomingFragmentPayload(const uint8_t* data, size_t size);
            bool isPending(uint32_t seq);
            size_t pendingCount() const;

        private:
            std::vector<std::vector<uint8_t>> buildFragments(uint32_t parentSeq, const std::vector<uint8_t>& payload);
            std::vector<uint8_t> buildFragmentPayload(uint32_t parentSeq, uint16_t fragId, uint16_t fragCount, const uint8_t* data, size_t len);
