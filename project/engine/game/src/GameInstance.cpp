...
{
    auto& posArray = _registry->getComponents<position_t>();
    auto& velArray = _registry->getComponents<position_t>();
    ...
    vel->vx = static_cast<float>(input[0]) * 100.f;
    vel->vy = static_cast<float>(input[1]) * 100.f;
}
...
Network::Packet System::GameInstance::produceSnapshot()
{
    ...
    for (size_t i = 0; i < posArray.size(); ++i) {
        ...
        float vx = velArray[i]->vx;
        float vy = velArray[i]->vy;
